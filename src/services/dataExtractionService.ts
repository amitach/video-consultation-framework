import { ConversationMessage, ExtractedData, DataExtractionRule } from '../types';
import { healthcarePersonaConfig, medicalContextMapping } from '../config/personas';

class DataExtractionService {
  private extractionRules: DataExtractionRule[];

  constructor() {
    this.extractionRules = this.buildExtractionRules();
  }

  private buildExtractionRules(): DataExtractionRule[] {
    const rules: DataExtractionRule[] = [];
  ...
  extractDataFromConversation(messages: ConversationMessage[]): ExtractedData {
  ...
  private applyExtractionRule(text: string, rule: DataExtractionRule): any {
  ...
  private extractMedicalContext(text: string, extractedData: ExtractedData): void {
    ...
    if (severityMatch) {
      ...
      if (severity >= 1 && severity <= 10) {
        extractedData.symptoms.severity = severity as 1 | 2 | 3 | 4 | 5;
      }
    }
  }
  ...
  private validateAndCleanData(extractedData: ExtractedData): void {
  ...
  extractFromMessage(message: ConversationMessage, currentData: ExtractedData): ExtractedData {
  ...
  generateDataSummary(extractedData: ExtractedData): string {
